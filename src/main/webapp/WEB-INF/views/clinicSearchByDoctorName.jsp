...
<body>
  ...
  <div class="back-to-top"></div>
    ...
    <c:set var="doctorName" value="${search }"></c:set>
	<div style="text-align: center; font-family: 'GowunBatang-Regular';">
		...
		<select name="searchOption"  onchange="searchBy(this)" >
			...
			<option value="clinicSearchByDoctorName" selected="selected">담당의</option>
	...
	<c:if test="${pg.startPage > pg.pageBlock}">
		<a class="btn btn-primary btn-sm" href="clinicSearchByDoctorName?search=${doctorName }&currentPage=${pg.startPage - pg.pageBlock }">[이전]</a>
	</c:if>
	<c:forEach var="i" begin="${pg.startPage }" end="${pg.endPage }">
		<a class="btn btn-primary btn-sm" href="clinicSearchByDoctorName?search=${doctorName }&currentPage=${i}">[${i}]</a>
	</c:forEach>
	<c:if test="${pg.endPage > pg.totalPage }">
		<a class="btn btn-primary btn-sm" href="clinicSearchByDoctorName?search=${doctorName }&currentPage=${pg.startPage + pg.pageBlock}">[다음]</a>
	</c:if>
	</div> 
